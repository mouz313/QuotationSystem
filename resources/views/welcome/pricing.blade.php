        ...
        <div class="grid md:grid-cols-3 gap-8 max-w-5xl mx-auto">
            @forelse($packages as $index => $package)
                @php
                    $isPopular = $index === 1;
                @endphp
                <div class="relative p-8 rounded-2xl border-2 transition {{ $isPopular ? 'border-indigo-600 shadow-xl shadow-indigo-100 scale-105' : 'border-gray-100 hover:border-indigo-200 hover:shadow-lg' }}">
                    @if($isPopular)
                    ...
                    <ul class="space-y-3 mb-8">
                        <li class="flex items-center gap-3 text-sm text-gray-600">
                            <x-icon name="check" class="w-4 h-4 text-indigo-500 shrink-0" />
                            {{ $package->max_users }} Team {{ Str::plural('User', $package->max_users) }}
                        </li>
                        <li class="flex items-center gap-3 text-sm text-gray-600">
                            <x-icon name="check" class="w-4 h-4 text-indigo-500 shrink-0" />
                        ...
                        <li class="flex items-center gap-3 text-sm text-gray-600">
                            <x-icon name="check" class="w-4 h-4 text-indigo-500 shrink-0" />
                            {{ $package->duration_days }}-day billing cycle
                        </li>
                        <li class="flex items-center gap-3 text-sm text-gray-600">
                            <x-icon name="check" class="w-4 h-4 text-indigo-500 shrink-0" />
                            PDF export & reports
                    ...
                    @if(Route::has('register'))
                        <a href="{{ route('register') }}" class="block w-full py-3 text-center text-sm font-semibold rounded-xl transition {{ $isPopular ? 'bg-indigo-600 text-white hover:bg-indigo-700' : 'bg-gray-100 text-gray-700 hover:bg-gray-200' }}">
                            Get Started
                        </a>
                    @endif
                </div>
