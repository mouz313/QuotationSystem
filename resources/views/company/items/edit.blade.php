...
<div class="bg-white rounded-xl shadow p-6 max-w-2xl">
    <form method="POST" action="/items/{{ $item->id }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="text" name="title" value="{{ old('title', $item->title) }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none">
        </div>
        <div>
        ...
        <div>
            ...
            <input type="number" name="unit_price" step="0.01" min="0" value="{{ old('unit_price', $item->unit_price) }}" required class="w-full px-4 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500 outline-none">
        </div>
