...
<div class="fade-in">
    <div class="toolbar">
        <div>
            <h1 style="font-size:1.25rem;font-weight:800;color:var(--gray-900);letter-spacing:-0.02em;">Notifications</h1>
            <p style="font-size:.8125rem;color:var(--gray-400);margin-top:.125rem;">All your activity updates in one place</p>
        </div>
    ...
    @if($unreadCount > 0)
    <div style="margin-bottom:1rem;padding:.875rem 1.25rem;border-radius:var(--radius-lg);background:var(--brand-50);border:1px solid var(--brand-100);display:flex;align-items:center;gap:.75rem;">
        ...
        <span style="font-size:.8125rem;font-weight:600;color:var(--brand-800);">You have {{ $unreadCount }} unread notification{{ $unreadCount > 1 ? 's' : '' }}</span>
    </div>
    @endif
        ...
        @forelse($notifications as $notif)
            @php
                $iconBg = match($notif->type) {
                    'payment_submitted' => 'background:var(--amber-50);color:var(--amber-600)',
                    'payment_approved' => 'background:var(--emerald-50);color:var(--emerald-600)',
                    'payment_rejected' => 'background:var(--red-50);color:var(--red-600)',
                    'status_changed' => 'background:var(--blue-50);color:var(--blue-600)',
            ...
            @endphp
            <a href="{{ $notif->url ?? '#' }}" style="display:flex;align-items:flex-start;gap:.875rem;padding:1rem 1.25rem;border-bottom:1px solid var(--gray-50);transition:background .15s;{{ !$notif->is_read ? 'background:var(--brand-50);' : '' }}" onmouseover="this.style.background='var(--gray-50)'" onmouseout="this.style.background='{{ !$notif->is_read ? 'var(--brand-50)' : 'transparent' }}'">
                ...
                <div style="flex:1;min-width:0;">
                    <p style="font-size:.8125rem;color:var(--gray-800);{{ $notif->is_read ? '' : 'font-weight:600;' }}">{{ $notif->message }}</p>
                    <p style="font-size:.75rem;color:var(--gray-400);margin-top:.25rem;">{{ $notif->created_at->diffForHumans() }} · {{ $notif->created_at->format('d M Y g:i A') }}</p>
                </div>
                @if(!$notif->is_read)
