...
<div class="fade-in">
    <x-page-header title="Notifications" subtitle="All your activity updates in one place">
        @slot('actions')
            @if(auth('client')->user()->notifications()->where('is_read', false)->exists())
                <form method="POST" action="/client/notifications/mark-all-read">
                    @csrf
...
@php $unreadCount = auth('client')->user()->unreadNotificationsCount(); @endphp
@if($unreadCount > 0)
<div class="fade-in" style="margin-bottom:1.25rem;padding:1rem;border-radius:.75rem;background:var(--brand-50);border:1px solid var(--brand-100);display:flex;align-items:center;gap:.75rem;">
