...
<body>
    <div class="wrapper">
        <div class="card">
            <div class="header">
                <h1>Quotation from {{ config('app.name') }}</h1>
                <p>Quote #{{ $quoteNumber }}</p>
            </div>
            <div class="body">
                <p style="font-size:15px;color:#374151;">Dear {{ $clientName }},</p>
                <p style="font-size:14px;color:#6b7280;">You have received a quotation from <strong>{{ config('app.name') }}</strong>. Please review the details below.</p>
                ...
                <div style="text-align:center;margin:24px 0;">
                    <a href="{{ url('/client/login') }}" class="btn btn-green">View in Client Portal</a>
                </div>

                <p style="font-size:13px;color:#9ca3af;text-align:center;">
                    You can log in to the client portal to view, accept, decline, or request changes to your quotations.<br>
                    <a href="{{ url('/client/login') }}" style="color:#4f46e5;">{{ url('/client/login') }}</a>
                </p>
            </div>
        </div>
